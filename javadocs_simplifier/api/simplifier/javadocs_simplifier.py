...
from ..decorators import memoize, export
...
class CSS:
    ...
    TYPE_NAME_LABEL: str = 'typeNameLabel'
...
class Field:
    ...
    def generate_stub(self) -> None:
        ...
        self.stub = "  " + self.stub.replace('java.lang.', '').replace('\n', '\n  ')
...
def run(soup: BS, field_type: str) -> None:
    ...
    for field in fields:
        ...
        if field_type is CSS.FIELD_DETAILS:
            ...
            stub += f'  {_.modifiers} {_.return_type} {_.name}; {description}\n'
        else:
            stub += f'{_.stub}\n'
        stub += '\n'
...
def generate(url: str) -> None:
    try:
        ...
        soup = get_soup(source, 'lxml')
        type_name = soup.find(
            HTML.SPAN,
            {
                CSS.CLASS: CSS.TYPE_NAME_LABEL,
            }
        ).string
        stub = f'public class {type_name} {{\n'
        stub += run(soup, CSS.FIELD_DETAILS)
        stub += run(soup, CSS.CONSTRUCTOR_DETAILS)
        stub += run(soup, CSS.METHOD_DETAILS)
        stub += '}\n'
        return (True, type_name, stub)
    except Exception:
        return (False, None, None)


# print(generate('https://cs300-www.cs.wisc.edu/wp/wp-content/uploads/2020/12/spring22/p5/doc/DraggableObject.html', 'lxml'))
